...
AI PIPELINE (3-layer hybrid):
  ...
  Layer 2 — Supervised ML (TF-IDF + Logistic Regression)
             Trained on CLINC150 dataset (~15 k samples, 150 intents)
             Handles general conversational intent classification
  Layer 3 — MyMemory AI Fallback (External free REST API)
             ...
             ensuring the student always gets a useful answer.
...
MODULES (all support Python 3.13+):
  flask, flask-cors, pandas, scikit-learn, requests
=============================================================
"""
...
from sklearn.pipeline import Pipeline                        # chains steps
from sklearn.preprocessing import LabelEncoder               # encode intents
import warnings
...
# =============================================================
# SECTION 1 — STUDENT IDENTITY   ← EDIT THESE VALUES
# =============================================================
STUDENT_PROFILE = {
    "full_name"  : "ANYADIKE SHARON NMESOMACHUKWU",            # your full name
    "reg_number" : "20231364542",                       # registration number
...
# =============================================================
# SECTION 2 — FUTO KNOWLEDGE BASE (Custom University DB)
# =============================================================
# Dict: intent_key -> list[str]  (random.choice picks one per reply)
# =============================================================
FUTO_KB = {
    ...
    "bot_identity": [
        (
            ...
            "  trained on CLINC150 (15,000+ intent samples)\n"
...
# =============================================================
MYMEMORY_EMAIL = "futobot@futo.edu.ng"   # 10k chars/day free
MYMEMORY_CACHE: dict = {}                # simple in-memory cache


def query_mymemory(user_text: str) -> "str | None":
    """
    Query the MyMemory REST API as an AI fallback.

    Endpoint: https://api.mymemory.translated.net/get
    Trick: langpair=en|en returns best semantic memory match —
    effectively a free general-knowledge lookup.
    ...
    try:
        ...
        # Reject if API just echoed the query back (no real match)
...
# =============================================================
# SECTION 5 — ML MODEL  (Layer 2)
# =============================================================
def load_and_train_model(csv_path: str):
    """
    Train TF-IDF + Logistic Regression on CLINC150 data.

    TF-IDF converts text to numeric vectors (ngram_range=(1,2)
    captures both single words and two-word phrases).
    LogisticRegression (multinomial) learns decision boundaries
    across 150 intent classes using the lbfgs solver.

    Returns: (pipeline, label_encoder, val_accuracy)
    """
    log.info("Loading CLINC150 dataset …")
    df       = pd.read_csv(csv_path)
    train_df = df[(df["split"] == "train") & (df["intent"].astype(str) != "0")].copy()
    val_df   = df[df["split"] == "val"].copy()

    log.info(f"Train: {len(train_df):,} | Val: {len(val_df):,} | Intents: {train_df['intent'].nunique()}")
...
CLINC_INTENT_TO_FUTO = {
    110: "timetable",           # meeting_schedule
    118: "timetable",           # schedule_meeting
    116: "course_registration", # pto_request (closest FUTO match)
    93 : "general_greeting",    # time
    96 : "general_greeting",    # timer
    127: "bot_identity",        # what_can_i_help_you_with
    135: "bot_identity",        # what_are_your_hobbies
    121: "bot_identity",        # who_made_you
    123: "bot_identity",        # are_you_a_bot
    131: "general_greeting",    # greeting
    129: "farewell",            # goodbye
    130: "thanks",              # thank_you
    132: "general_greeting",    # tell_me_a_joke
    141: "unknown",
    150: "unknown",
    139: "unknown",
}
...
def classify_intent(user_text: str, pipeline, label_enc) -> str:
    """Classify intent using Layer 1 (rules) then Layer 2 (ML)."""
...
try:
    ML_PIPELINE, LABEL_ENC, MODEL_ACC = load_and_train_model(CSV_PATH)
    MODEL_READY = True
    log.info("ML model ready!")
except FileNotFoundError:
    log.warning(f"'{CSV_PATH}' not found — keyword-only + MyMemory mode.")
except Exception as exc:
...
@app.route("/")
def index():
    # Pre-render student profile and model_acc into the HTML,
    # then serve as plain string to avoid Jinja2 parsing CSS {{ }} blocks
    html = HTML_TEMPLATE
...
def feedback():
    """Accept thumbs-up / thumbs-down from the UI."""
    data = request.get_json(force=True)
    if data.get("vote") == "up":
        ANALYTICS["upvotes"] += 1
